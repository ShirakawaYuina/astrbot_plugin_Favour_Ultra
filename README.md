...
[![查看更新日志（v3.1.10）](https://img.shields.io/badge/查看更新日志-blue?style=for-the-badge)](#更新日志)
...
* 用户印象现在只基于用户与机器人的实际完整一问一答，不再混入相邻群友发言。
* 印象样本按用户与会话隔离；复读类印象需要至少三轮用户侧独立重复证据。
* 已有印象不会被批量修改，将在后续达到自然对话轮数阈值时按新规则刷新。
* 请务必在更新后重载插件以应用新的采样逻辑。
...
## 💻 命令使用手册
- 聊天内可使用 `/好感度帮助` 查看当前账号权限下的可用命令。
- 旧版 `查询好感度`、`查询当前好感度`、`查询全部好感度`、`查询全局好感度` 仍可继续使用，但推荐使用下方短指令。

### 1. 通用命令 (所有用户)

| 命令 | 示例 | 描述 |
| :--- | :--- | :--- |
| `好感度 [@用户或用户ID]` | `好感度` / `好感度 @某人` | **[文本]** 查询自己或指定用户的好感度、关系、今日变化和印象。 |
| `当前好感度 [页码]` | `当前好感度 2` | **[图片]** 分页生成当前会话的好感度排行。全局模式下请改用 `全局好感度`。 |
| `全局好感度 [页码]` | `全局好感度 1` | **[图片]** 分页查询全局好感度记录；群聊中只显示当前群成员记录。 |
| `查询惩罚列表` | `查询惩罚列表` | **[文本]** 查看当前因刷好感检测进入惩罚期的用户。 |
| `好感度帮助` | `好感度帮助` | 查看当前账号权限下可用的命令菜单。 |
| `好感度指令帮助` | `好感度指令帮助` | 查看详细指令格式、示例和限制说明。 |
...
| 命令 | 权限 | 描述 |
| :--- | :--- | :--- |
| `修改好感度 <@用户或用户ID> <数值>` | 群管+ | 修改指定用户在当前会话中的好感度；目标用户必须已有记录。 |
| `清空好感度 <@用户或用户ID>` | 群主+ | 清空指定用户在当前会话中的好感度数据，需在 30 秒内二次确认。 |
...
| 命令 | 描述 |
| :--- | :--- |
| `全部好感度` | **[图片]** 按会话分组展示所有非全局数据。*注：在群聊中使用时会自动隐藏私聊会话数据。* |
| `全局修改好感度 <@用户或用户ID> <数值>` | 修改指定用户在**所有会话**中的好感度；目标用户必须已有记录。 |
| `跨会话修改 <SID> 修改好感度 <用户ID> <数值>` | 远程修改指定会话中指定用户的好感度。示例：`/跨会话修改 group:123456 修改好感度 10001 50` |
| `清空全局好感度` | **[极度慎用]** 清空数据库中的全部好感度记录，需回复 `确认清空所有数据`。 |
...
### 2. 智能 T2I 报表
当查询多人数据（如 `当前好感度`）时，插件会自动调用 AstrBot 的文本转图片引擎，生成包含 Markdown 表格的图片。
